...
function createCard(p){
  const el=document.createElement("article");el.className=`pack-card${p.owned?"":" is-missing"}`;
  el.style.setProperty("--pack-a",palettes[p.era][0]);el.style.setProperty("--pack-b",palettes[p.era][1]);
  el.innerHTML=`<div class="pack-art"><div class="pack-symbol"><span>${p.era}</span></div></div><div class="pack-card-body"><div class="pack-card-top"><span class="pack-code">${p.code}</span><span class="pack-status">${p.owned?"수집완료":"미수집"}</span></div><strong class="pack-name">${p.name}</strong></div>`;
  return el;
}
function render(){
  ...
  ["S","SV","M"].forEach(key=>{
    ...
    section.innerHTML=`<div class="pack-series-heading"><h3>${key} 시리즈</h3><p>${items.filter(p=>p.owned).length} / ${items.length}팩 수집완료</p></div><div class="pack-grid"></div>`;
    const grid=section.querySelector(".pack-grid");items.forEach(p=>grid.append(createCard(p)));host.append(section);
  });
